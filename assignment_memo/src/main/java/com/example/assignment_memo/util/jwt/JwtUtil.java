...
public class JwtUtil {
    ...
    private final UserDetailsServiceImpl userDetailsService;                                                            // UserDetailService 구현 클래스 주입
    ...
    public void init() {
        ...
        // 1. Base64로 인코딩 되어있는 문자열을 byte 배열로 변환
    ...
    public String resolveToken(HttpServletRequest request){                                                             // Http프로토콜의 request정보를 서블릿에게 전달하기 위한 목적으로 사용하는 매개변수
        ...
        if(StringUtils.hasText(bearerToken) && bearerToken.startsWith(BEARER_PREFIX)){                                  // StringUtils.hasText는 값이 있을경우 true , 공백이거나 Null이 들어온 경우 false반환 / bearerToken.startsWith는 필드값이 해당 변수와 동일한지 확인
    ...
    public boolean validateToken(String token){
        try {
                                                                                                                        ...
                                                                                                                        // 서버의 시크릿키로 서명한 것을 토큰화한 jws인것인지 검증
    ...
    public Authentication createAuthentication(String username) {
        ...
        return new UsernamePasswordAuthenticationToken(userDetails, null, userDetails.getAuthorities());     // 이 토큰은 신뢰된 인증 토큰이 만족된 AuthenticationManager 또는 AuthenticationProvider 구현객체에 의해 사용
    }
}
