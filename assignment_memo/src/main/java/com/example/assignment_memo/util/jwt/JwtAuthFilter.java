...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    public void setAuthentication(String username) {
        ...
        // 1. Authenication 생성                                                                                        // 인증 객체를 만드는걸 jwtUtil로 책임분리
        Authentication authentication = jwtUtil.createAuthentication(username);
        // 2. SecurityContext에 세팅
    ...
    // 이부분에 대한 에러 처리가 아직 미완성. 커스텀한 예외처리로 응답 반환이 안된다.
    public void jwtExceptionHandler(HttpServletResponse response, String msg, int statusCode) {                         // 토큰이 오류가 나면 메세지를 커스터 마이징해 로그로 남기거나 응답메세지로
