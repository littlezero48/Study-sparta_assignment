...
//@EnableGlobalMethodSecurity 스프링 시큐리티의 메소드 어노테이션 기반 시큐리티를 활성화
//securedEnabled = @Secured어노테이션 활성화 여부 @Secured는 접근 권한 부여하는 어노테이션
@EnableGlobalMethodSecurity(securedEnabled = true) // @Secured 어노테이션 활성화
public class WebSecurityConfig {
    private final JwtUtil jwtUtil; // 의존성 주입

    @Bean // 의존성 주입으로 사용할 수 있게 Bean으로 등록 / 설정을 애플리케이션 전역에 쓰려고?
    //스프링 시큐리티에서 제공하는 인터페이스 객체로 비밀번호를 암호화 한다.
    ...
    public WebSecurityCustomizer webSecurityCustomizer(){ //접근 허용 설정
        return (web) -> web.ignoring() // 보안 필터에서 제외할 것
                ...
                // 요청일치 : 경로요청, 스트링부트에서 제공하는 정적 리소스로의
